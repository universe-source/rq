...
class Queue(object):
    ...
    def enqueue_call(self, func, args=None, kwargs=None, timeout=None,
        ...
        # 2 利用类方法create来完成Job的实例化
        MY(10.1, __file__, 'Enqueue, type(job_class):', self.job_class)
        ...
        # 3 此时job.id仍旧为空, 但是访问job.id时, 调用get_id, 默认生成id
        MY(10.2, __file__, 'Instant Job, job_id:', job.id)
    ...
    def enqueue_job(self, job, pipeline=None, at_front=False):
        ...
        MY(10.3, __file__,
           '\n\tadd {} to {}'.format(self.key, self.redis_queues_keys),
           '\n\tjob key:', job.key,
           '\n\tttl:', job.ttl,
           '\n\tresult_ttl:', job.result_ttl)

        # 3 将数据都存储到self.key, 相当于入队列(dict)
        job.save(pipeline=pipe)
        job.cleanup(ttl=job.ttl, pipeline=pipe)
    ...
    def dequeue_any(cls, queues, timeout, connection=None, job_class=None):
        ...
        either blocks execution of this function for the duration期间 of the
        ...
        while True:
            ...
            MY(3, __file__, 'Queues keys, 进入timeout阶段:', queue_keys)
            ...
            queue = cls.from_queue_key(queue_key,
                                       connection=connection,
                                       job_class=job_class)
            MY(3.0, __file__, '未发生timeout, Get task and instantiation',
               '\n\tqueue_key:', queue_key,
               '\n\tkey', queue.key)
            try:
                import json
                job = job_class.fetch(job_id, connection=connection)
                MY(3.1, __file__, 'Instantiation Job and get data:',
                   '\n\tkey:', job.key,
                   '\n\tstatus:', job.status,
                   '\n\tttl:', job.ttl,
                   )

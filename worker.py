...
class Worker(object):
    ...
    def register_birth(self):
        ...
        with self.connection._pipeline() as p:
            ...
            # 注册worker到系统, 设置如下键以及值
            #   rq:workers(值为rq:worker:PC-NAME:PID)
            #   rq:workers:QUEUE_NAME(多个队列名称)
            worker_registration.register(self, p)
            # 该过期时间在heartbeat中会被重置
    ...
    def work(self, burst=False, logging_level="INFO"):
        ...
        """
        # 0 设置日志等级, 注册 SIGTERM, SIGINT信号
        setup_loghandlers(logging_level)
        self._install_signal_handlers()

        did_perform_work = False
        # 1 向redis中写入worker信息: rq:worker:PC-NAME-PID, 并设置过期时间(420)
        # 2 redis中写入: rq:worker:QUEUE_NAME
        self.register_birth()
        ...
        try:
            MY(2, __file__, 'Worker进入 Loop 无线循环状态(heartbeat).') 
            while True:
                try:
                    ...
                    if self._stop_requested:
                        ...
                        break
                    #  1 获取能够超时的最大时间, 避免worker直接过期了, 其中worker
                    #   的ttl在beartbeat中重置
                    timeout = None if burst else max(1, self.default_worker_ttl - 60)
                    #  2 遍历所有的queues(rq:workers:QUEUE_NAME), 直到获取task或者遍历结束
                    #  2.1 当且仅当timeout==None时, result才会返回空
                    #  2.2 如果queues中没有task, 则一直在该函数中,不断的timeout
                    #  2.3 如果返回, 那么就表示肯定存在task
                    ...
                    if result is None:
                        ...
                        break
                    # 3 处理 task
                    # 3.1 设置worker状态(idle->busy)
                    # 3.2 fork子进程
                    # 3.3 父进程监听子进程状态
                    # 3.4 处理完成, 重新设置worker状态(busy->idle)
                    job, queue = result
    ...
    def dequeue_job_and_maintain_ttl(self, timeout):
        ...
        # 无线循环用于避免因为网络等其他原因, 获取队列超时, 能够继续获取
        while True:
            # 1 重新设置过期时间
            self.heartbeat()

            # 2 循环遍历, 从所有queues中获取第一个有效的task(job)
            try:
    ...
    def heartbeat(self, timeout=0, pipeline=None):
        ...
        only larger.
        重新设置worker的过期时间, 避免rq:worker:PC:PID被删除
        """
    ...
    def _monitor_work_horse_tick(self, job):
        """当且仅当子进程没有正常返回的时候, 才会将queue放入到failed队列中"""
        ...
        if job_status is None:  # Job completed and its ttl has expired
            return
        # 注意非finished/failed状态, 表示子进程可能已经成为僵尸进程了
    ...
    def prepare_job_execution(self, job):
        ...
        with self.connection._pipeline() as pipeline:
            # 1 设置rq:worker:PC-NAME:pid的状态
            self.set_state(WorkerStatus.BUSY, pipeline=pipeline)
            # 2 设置rq:worker:PC-NAME:PID中的current_job键值
            self.set_current_job_id(job.id, pipeline=pipeline)
            self.heartbeat(timeout, pipeline=pipeline)
            # 3 实例化, 这里会创建rq:registry:(job.origin)键值
            registry = StartedJobRegistry(job.origin,
                                          self.connection,
                                          job_class=self.job_class)
            # 4 创建task: rq:job:uuid, 并设置过期时间
